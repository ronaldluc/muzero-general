...
ACTIONS = list(product(np.arange(start=-1, stop=1, step=0.1),
                       np.arange(0, 1, 0.1),
                       np.arange(0, 1, 0.1),
                       ))
...
class MuZeroConfig:
    def __init__(self):
        ...
        self.max_moves = 600  # Maximum number of moves if game is not finished before
...
class Game(AbstractGame):
    ...
    def step(self, action):
        ...
        return np.swapaxes(observation, 0, 2), reward, done
    ...
    def action_to_string(self, action_number):
        ...
        return f"steer: {action[0]} gas: {action[1]} break: {action[2]}"
...
class TilingEnv(gym.Env, EzPickle):
    ...
    def render(self, mode="human"):
        ...
        in_degrees = -np.rad2deg(x[2])
        print(f'Angle: {in_degrees:4.2} Cart X: {cartx:4.2f}')
